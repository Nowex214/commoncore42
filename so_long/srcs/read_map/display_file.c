...
/*   Updated: 2024/11/28 13:28:42 by ehenry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		while (game->map[y][x])
		{
			if (game->map[y][x] == '0')
				mlx_put_image_to_window(game->mlx_environment,
					game->display_window, game->ground, x * 32 , y * 32);
			if (game->map[y][x] == '1')
				mlx_put_image_to_window(game->mlx_environment,
					game->display_window, game->wall, x * 32 , y * 32);
			if (game->map[y][x] == 'P')
				mlx_put_image_to_window(game->mlx_environment,
					game->display_window, game->player, x * 32, y * 32);
			if (game->map[y][x] == 'E')
				mlx_put_image_to_window(game->mlx_environment,
					game->display_window, game->exit, x * 32, y * 32);
			if (game->map[y][x] == 'C')
				mlx_put_image_to_window(game->mlx_environment,
					game->display_window, game->collectable, x * 32, y * 32);
			x++;
		}
		y++;
	}
